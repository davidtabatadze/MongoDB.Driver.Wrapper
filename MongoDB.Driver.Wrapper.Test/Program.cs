    ...
    class Program
    {
        ...
        void UnitTests(IMongoContext ctx)
        {
            ...
            Log(
                ...
                new TestEntityLong { }.Key >= 0 && // უნდა დაგენერირდეს დადებითი გასაღები [რიცხვი]
                new TestEntityString { }.Key.HasValue() // უნდა დაგენერირდეს რაიმე გასაღები [სტრიქონი]
            );
            // *****************************************************************************
        ...
        {// ??? სტრიქონ-გასაღებებზე აუცილებელია ToLower() ???
            ...
            try
            {
                ...
                {
                    Server = "mongodb://discovery_api_user:b1lWn1BqVyZBalsdjdRA0BPV31DxYqL6@production.mongodb-01.private.optio.ai:27017/Discovery?authSource=Discovery&replicaSet=rs-main&readPreference=primaryPreferred&appname=MongoDB%20Compass&ssl=false",
                    Database = "Discovery",
                ...
                var ctx = new MongoContext(config);

                ctx.Mappings(mapping);

                ctx.Delete<TestEntityLong>();
                ctx.Delete<TestEntityString>();
                ctx.Insert(new TestEntityLong { Key = 1 });
                ctx.Insert(new TestEntityString { Key = "1" });
